...
namespace tvn_cosine.ai.test.learning.fol.kb.data
{
    [TestClass]
    public class ClauseTest
    {
        ...
        public void testBinaryResolvents()
        {
            ...
            Assert.IsTrue(c1.binaryResolvents(c1).First().isEmpty());
        ...
        public void testBinaryResolventsOrderDoesNotMatter()
        {
            ...
            foreach (var v in kb.getAllClauses())
                clauses.Add(v);

            ISet<Clause> newClauses = new HashSet<Clause>();
            long maxRunTime = 30 * 1000; // 30 seconds
            DateTime finishTime = System.DateTime.Now.AddMilliseconds(maxRunTime);
            do
            {
                ...
                for (int i = 0; i < clausesA.Length; i++)
                {
                    ...
                    for (int j = 0; j < clausesA.Length; j++)
                    {
                        ...
                        foreach (var v in cI.getFactors())
                            newClauses.Add(v);
                        foreach (var v in cJ.getFactors())
                            newClauses.Add(v);
                        ...
                        if (!cIresolvents.Equals(cJresolvents))
                        {
                            ...
                            Console.WriteLine("cIR=" + cIresolvents);
                            Console.WriteLine("cJR=" + cJresolvents);
