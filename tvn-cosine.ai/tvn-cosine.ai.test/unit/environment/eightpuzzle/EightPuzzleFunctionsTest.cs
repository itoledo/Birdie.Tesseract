...
namespace tvn_cosine.ai.test.unit.environment.eightpuzzle
{
    ...
    public class EightPuzzleFunctionsTest
    {
        ...
        {
            IQueue<IAction> actions = Factory.CreateQueue<IAction>(EightPuzzleFunctions.getActionsFunction().apply(board));
            Assert.AreEqual(3, actions.Size());

            // test first successor
            EightPuzzleBoard expectedFirst = new EightPuzzleBoard(new int[] { 1, 2,
                0, 3, 4, 5, 6, 7, 8 });
            EightPuzzleBoard actualFirst = (EightPuzzleBoard)EightPuzzleFunctions.getResultFunction().apply(board, actions.Get(0));
            ...
            EightPuzzleBoard expectedSecond = new EightPuzzleBoard(new int[] { 1,
                2, 5, 3, 4, 8, 6, 7, 0 });
            EightPuzzleBoard actualSecond = (EightPuzzleBoard)EightPuzzleFunctions.getResultFunction().apply(board, actions.Get(1));
            ...
            EightPuzzleBoard expectedThird = new EightPuzzleBoard(new int[] { 1, 2,
                5, 3, 0, 4, 6, 7, 8 });
            EightPuzzleBoard actualThird = (EightPuzzleBoard)EightPuzzleFunctions.getResultFunction().apply(board, actions.Get(2));
        ...
        public void testGenerateCorrectWhenGapMovedRightward()
        {
            ...
            EightPuzzleBoard expectedFourth = new EightPuzzleBoard(new int[] { 1,
                2, 5, 3, 4, 0, 6, 7, 8 });
            EightPuzzleBoard actualFourth = (EightPuzzleBoard)EightPuzzleFunctions.getResultFunction().apply(board, actions.Get(3));
