...
namespace tvn.cosine.ai.search.framework.qsearch
{
    ...
    public class GraphSearchReducedFrontier<S, A> : QueueSearch<S, A>
    {
        ...
        private IDictionary<S, Node<S, A>> frontierNodeLookup = new Dictionary<S, Node<S, A>>();
        ...
        public override Node<S, A> findNode(IProblem<S, A> problem, IQueue<Node<S, A>> frontier)
        {
            // initialize the explored set to be empty
            if (frontier is PriorityQueue<Node<S, A>>)
                nodeComparator = ((PriorityQueue<Node<S, A>>)frontier).Comparer;
            explored.Clear();
        ...
        protected override void addToFrontier(Node<S, A> node)
        {
            ...
            {
                Node<S, A> frontierNode = frontierNodeLookup[node.getState()];
