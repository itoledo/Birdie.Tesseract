...
namespace tvn.cosine.ai.search.framework.qsearch
{
    ...
    public abstract class QueueSearch<S, A>
    {
        ...
        protected readonly NodeExpander<S, A> nodeExpander;
         ...
         * first search.
         */
        public virtual void setEarlyGoalTest(bool b)
        {
            earlyGoalTest = b;
        }

        public virtual NodeExpander<S, A> getNodeExpander()
        {
            return nodeExpander;
        }
        ...
        protected void clearMetrics()
        {
            ...
            metrics[METRIC_QUEUE_SIZE] = 0;
            metrics[METRIC_MAX_QUEUE_SIZE] = 0;
            metrics[METRIC_PATH_COST] = 0;
        }

        protected void updateMetrics(int queueSize)
        {
            metrics[METRIC_QUEUE_SIZE] = queueSize;
            int maxQSize = (int)metrics[METRIC_MAX_QUEUE_SIZE];
            if (queueSize > maxQSize)
            {
                metrics[METRIC_MAX_QUEUE_SIZE] = queueSize;
            }
        }
