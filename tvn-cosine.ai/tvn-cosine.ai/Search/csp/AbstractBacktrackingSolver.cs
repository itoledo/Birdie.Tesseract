...
namespace tvn.cosine.ai.search.csp
{
        ...
        where VAR : Variable
    {
        ...
        {
            return backtrack(csp, new Assignment<VAR, VAL>(), cancellationToken);
        }
        ...
        //{
        //    return backtrack(csp, assignment, CancellationToken.None);
        //}
         ...
         */
        private Assignment<VAR, VAL> backtrack(CSP<VAR, VAL> csp, Assignment<VAR, VAL> assignment, CancellationToken cancellationToken)
        {
            Assignment<VAR, VAL> result = null;
            if (assignment.isComplete(csp.getVariables()) || cancellationToken.IsCancellationRequested)
            {
                result = assignment;
            }
            else
            {
                ...
                foreach (VAL value in orderDomainValues(csp, assignment, var))
                {
                    ...
                    if (assignment.isConsistent(csp.getConstraints(var)))
                    {
                        ...
                        {
                            result = backtrack(csp, assignment, cancellationToken);
                            if (result != null)
                                break;
                        }
                        log.undo(csp);
