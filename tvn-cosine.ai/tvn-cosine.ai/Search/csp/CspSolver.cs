...
namespace tvn.cosine.ai.search.csp
{
    ...
    public abstract class CspSolver<VAR, VAL>
        where VAR : Variable
    {
        private List<CspListener<VAR, VAL>> listeners = new List<CspListener<VAR, VAL>>();
        ...
        {
            return solve(csp, CancellationToken.None);
        }

        public abstract Assignment<VAR, VAL> solve(CSP<VAR, VAL> csp, CancellationToken token);
