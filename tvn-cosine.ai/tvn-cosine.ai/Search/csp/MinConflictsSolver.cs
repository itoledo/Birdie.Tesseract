...
namespace tvn.cosine.ai.search.csp
{
        ...
        where VAR : Variable
    {
        ...
        public override Assignment<VAR, VAL> solve(CSP<VAR, VAL> csp, CancellationToken cancellationToken)
        {
            ...
            for (int i = 0; i < maxSteps && !cancellationToken.IsCancellationRequested; ++i)
            {
                ...
                else
                {
                    ...
                    VAR var = Util.selectRandomlyFromSet<VAR>(vars);
        ...
        {
            ISet<VAR> result = new HashSet<VAR>();
            foreach (var constraint in csp.getConstraints().Where(constraint => !constraint.isSatisfiedWith(assignment)))
            {
                foreach (var v in constraint.getScope().Where(var => !result.Contains(var)))
                {
                    result.Add(v);
                }
            }
        ...
        private VAL getMinConflictValueFor(VAR var, Assignment<VAR, VAL> assignment, CSP<VAR, VAL> csp)
        {
            ...
            List<VAL> resultCandidates = new List<VAL>();
            ...
            return (resultCandidates.Count > 0) ? Util.selectRandomlyFromList(resultCandidates) : default(VAL);
        }
