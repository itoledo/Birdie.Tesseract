...
namespace tvn.cosine.ai.util.math.geom.shapes
{
     ...
     *
     */
    public sealed class Polyline2D : IGeometric2D
    {
        ...
        public IGeometric2D transform(TransformMatrix2D matrix)
        {
            ...
            return new Polyline2D(vertexesNew, _isClosed
                                            || (Util.compareDoubles(vertexesNew[0].getX(), vertexesNew[vertexes.Length - 1].getX())
                                             && Util.compareDoubles(vertexesNew[0].getY(), vertexesNew[vertexes.Length - 1].getY())));
        }
    }
