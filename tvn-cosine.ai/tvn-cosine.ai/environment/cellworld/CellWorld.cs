...
public class CellWorld<C> {
	private ISet<Cell<C>> cells = new HashSet<Cell<C>>();
	private IDictionary<Integer, IDictionary<Integer, Cell<C>>> cellLookup = new Dictionary<Integer, IDictionary<Integer, Cell<C>>>();
	...
	public CellWorld(int xDimension, int yDimension, C defaultCellContent) {
		for (int x = 1; x <= xDimension; x++) {
			IDictionary<Integer, Cell<C>> xCol = new Dictionary<Integer, Cell<C>>();
	...
	public void removeCell(int x, int y) {
		IDictionary<Integer, Cell<C>> xCol = cellLookup.get(x);
		if (null != xCol) {
