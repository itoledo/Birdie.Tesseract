...
public class HybridWumpusAgent : AbstractAgent {
	 ...
	 */
	public List<Action> planRoute(AgentPosition current, ISet<Room> goals, ISet<Room> allowed) {
		...
		try {
			...
			// no actions.
			if (actions.Count == 1 && actions.get(0).isNoOp()) {
				actions = new List<>();
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 */
	public List<Action> planShot(AgentPosition current, ISet<Room> possibleWumpus, ISet<Room> allowed) {
		...
		if (actions.Count > 0) {
			newPos = ((Forward) actions.get(actions.Count - 1)).getToPosition();
		}
