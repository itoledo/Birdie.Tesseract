...
namespace tvn.cosine.ai.environment.wumpusworld
{
    ...
    public class WumpusKnowledgeBase : KnowledgeBase
    {
        ...
        public static readonly string ACTION_FORWARD = Forward.FORWARD_ACTION_NAME;
        public static readonly string ACTION_SHOOT = Shoot.SHOOT_ACTION_NAME;
        public static readonly string ACTION_TURN_LEFT = TurnLeft.TURN_LEFT_ACTION_NAME;
        public static readonly string ACTION_TURN_RIGHT = TurnRight.TURN_RIGHT_ACTION_NAME;
        ...
        { 
            ...
            for (int y = 1; y <= caveYDimension; y++)
            {
                ...
                {
                    IList<PropositionSymbol> pitsIn = new List<PropositionSymbol>();
                    IList<PropositionSymbol> wumpsIn = new List<PropositionSymbol>();
                    ...
                    tell(new ComplexSentence(newSymbol(BREEZE, x, y), Connective.BICONDITIONAL, Sentence.newDisjunction(pitsIn.ToArray())));
                    tell(new ComplexSentence(newSymbol(STENCH, x, y), Connective.BICONDITIONAL, Sentence.newDisjunction(wumpsIn.ToArray())));
                }
            }
            ...
            IList<PropositionSymbol> wumpsAtLeast = new List<PropositionSymbol>();
            for (int x = 1; x <= caveXDimension; x++)
            {
                for (int y = 1; y <= caveYDimension; y++)
                {
        ...
        public AgentPosition askCurrentPosition(int t)
        {
            ...
            if (locX == -1 || locY == -1)
            {
                throw new System.Exception("Inconsistent KB, unable to determine current room position.");
            }
            ...
            {
                throw new System.Exception("Inconsistent KB, unable to determine current room orientation.");
            }
        ...
        {
            ISet<Room> safe = new HashSet<Room>();
        ...
        {
            ISet<Room> unvisited = new HashSet<Room>();
        ...
        {
            ISet<Room> possible = new HashSet<Room>();
        ...
        {
            ISet<Room> notUnsafe = new HashSet<Room>();
        ...
        public void tellTemporalPhysicsSentences(int t)
        {
            ...
            for (int x = 1; x <= caveXDimension; x++)
            {
                for (int y = 1; y <= caveYDimension; y++)
                {
                    ...
                    // Location
                    List<Sentence> locDisjuncts = new List<Sentence>();
        ...
        {
            IList<Sentence> sentences = getSentences();
