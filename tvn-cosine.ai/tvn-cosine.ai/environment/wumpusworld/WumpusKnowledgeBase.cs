	...
	public WumpusKnowledgeBase(DPLL dpll, int caveXandYDimensions) {
		...
		for (int y = 1; y <= caveYDimension; y++) {
			for (int x = 1; x <= caveXDimension; x++) {
				...
				List<PropositionSymbol> pitsIn  = new List<PropositionSymbol>();
				List<PropositionSymbol> wumpsIn = new List<PropositionSymbol>();
		...
		List<PropositionSymbol> wumpsAtLeast = new List<PropositionSymbol>();
		for (int x = 1; x <= caveXDimension; x++) {
			for (int y = 1; y <= caveYDimension; y++) {
