 ...
 * 
 */
public class QLearningAgent<S, A : Action> extends
	...
	public A execute(PerceptStateReward<S> percept) {
		...
		if (isTerminal(sPrime)) {
			Q.Add(new Pair<S, A>(sPrime, noneAction), rPrime);
		}
	...
	private bool isTerminal(S s) {
		bool terminal = false;
		if (null != s && actionsFunction.actions(s).Count == 0) {
	...
	private double maxAPrime(S sPrime) {
		...
		if (actionsFunction.actions(sPrime).Count == 0) {
			// a terminal state
