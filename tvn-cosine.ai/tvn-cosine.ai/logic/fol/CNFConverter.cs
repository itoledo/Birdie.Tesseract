...
class StandardizeQuantiferVariables : FOLVisitor {
	...
	public object visitQuantifiedSentence(QuantifiedSentence sentence,
			Object arg) {
		Set<Variable> seenSoFar = (ISet<Variable>) arg;
...
class RemoveQuantifiers : FOLVisitor {
	...
	public object visitQuantifiedSentence(QuantifiedSentence sentence,
		...
		Set<Variable> universalScope = (ISet<Variable>) arg;
		...
		if (Quantifiers.isEXISTS(sentence.getQuantifier())) {
			IDictionary<Variable, Term> skolemSubst = new Dictionary<Variable, Term>();
...
class CNFConstructor : FOLVisitor {
	...
	public object visitPredicate(Predicate p, object arg) {
		...
		if (ad.negated) {
			ad.clauses.get(ad.clauses.Count - 1).addNegativeLiteral(p);
		} else {
			ad.clauses.get(ad.clauses.Count - 1).addPositiveLiteral(p);
		}
		return p;
	}
	...
	public object visitTermEquality(TermEquality equality, object arg) {
		ArgData ad = (ArgData) arg;
		if (ad.negated) {
			ad.clauses.get(ad.clauses.Count - 1).addNegativeLiteral(equality);
		} else {
			ad.clauses.get(ad.clauses.Count - 1).addPositiveLiteral(equality);
		}
		return equality;
	}
