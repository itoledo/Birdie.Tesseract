...
public class Unifier {
	...
	// else if LIST?(x) and LIST?(y) then
	// return UNIFY(x.REST, y.REST, UNIFY(x.FIRST, y.FIRST, theta))
	public IDictionary<Variable, Term> unify(List<? : FOLNode> x,
			List<? : FOLNode> y, IDictionary<Variable, Term> theta) {
		if (theta == null) {
			return null;
			...
			return unify(x.subList(1, x.Count), y.subList(1, y.Count),
	...
	protected bool occurCheck(IDictionary<Variable, Term> theta, Variable var,
			FOLNode x) {
		// ((equal var x) t)
		if (var .Equals(x)) {
			return true;
	...
	// for need for this.
	private IDictionary<Variable, Term> cascadeSubstitution(IDictionary<Variable, Term> theta,
			Variable var, Term x) {
		theta.Add(var, x);
		for (Variable v : theta.Keys) {
			theta.Add(v, _substVisitor.subst(theta, theta.get(v)));
		}
