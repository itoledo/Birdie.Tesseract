...
public class FOLModelElimination : InferenceProcedure {
	...
	public InferenceResult ask(FOLKnowledgeBase kb, Sentence query) {
		...
		Set<Clause> bgClauses = new HashSet<Clause>(kb.getAllClauses());
	...
	// PRIVATE METHODS
	//
	private List<Chain> createChainsFromClauses(ISet<Clause> clauses) {
...
class IndexedFarParents {
	...
	private SubstVisitor substVisitor = new SubstVisitor();
	//
	private IDictionary<String, List<Chain>> posHeads = new Dictionary<String, List<Chain>>();
	private IDictionary<String, List<Chain>> negHeads = new Dictionary<String, List<Chain>>();
	...
	public int getNumberFarParents(Chain farParent) {
		...
		IDictionary<String, List<Chain>> heads = null;
	...
	public void resetNumberFarParentsTo(Chain farParent, int toSize) {
		...
		while (farParents.Count > toSize) {
			farParents.Remove(farParents.Count - 1);
		}
	}
	...
	public int getNumberCandidateFarParents(Chain nearParent) {
		...
		IDictionary<String, List<Chain>> candidateHeads = null;
	...
	public Chain addToIndex(Chain c) {
		...
		Literal head = c.getHead();
		if (null != head) {
			IDictionary<String, List<Chain>> toAddTo = null;
			if (head.isPositiveLiteral()) {
				toAddTo = posHeads;
			} else {
				toAddTo = negHeads;
	...
	public override string ToString() {
		...
		for (string key : posHeads.Keys) {
			sb.Append(",");
			sb.Append(posHeads.get(key).Count);
		}
		sb.Append(" posHeads=");
		...
		for (string key : negHeads.Keys) {
			sb.Append(",");
			sb.Append(negHeads.get(key).Count);
		}
		sb.Append(" negHeads=");
