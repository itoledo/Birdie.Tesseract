...
public class Model : PLVisitor<Boolean, Boolean> {
	...
	private Dictionary<PropositionSymbol, Boolean> assignments = new Dictionary<PropositionSymbol, Boolean>();
	...
	public Model(IDictionary<PropositionSymbol, Boolean> values) {
	...
	public ISet<PropositionSymbol> getAssignedSymbols() {
		return new HashSet<>(assignments.Keys);
	}
	...
	public void print() {
		for (Map.Entry<PropositionSymbol, Boolean> e : assignments.entrySet()) {
			System.out.print(e.Key + " = " + e.getValue() + " ");
		}
		Console.WriteLine();
	}
