...
namespace tvn.cosine.ai.nlp.parsing
{
    ...
    public class CYK
    {
        ...
        public float[,,] parse(IList<string> words, ProbCNFGrammar grammar)
        {
            int N = length(words);
            int M = grammar.vars.Count;
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                {
                    Rule r = (Rule)grammar.rules[j];
                    if (r.derives(words[i]))
                    {                   // rule is of form X -> w, where w = words[i]
                        int x = grammar.vars.IndexOf(r.lhs[0]); // get the index of rule's LHS variable
            ...
            for (int length = 2; length <= N; length++)
            {
                for (int start = 1; start <= N - length + 1; start++)
                {
                    ...
                    { // N.B. the book incorrectly has N-1 instead of length-1
                        ...
                        for (int j = 0; j < grammar.rules.Count; j++)
                        {
                            Rule r = grammar.rules[j];
                            if (r.rhs.Count == 2)
                            {
                                // get index of rule's variables X, Y, and Z
                                int x = grammar.vars.IndexOf(r.lhs[0]);
                                int y = grammar.vars.IndexOf(r.rhs[0]);
                                int z = grammar.vars.IndexOf(r.rhs[1]);
         ...
         */
        public int length(IList<string> ls)
        {
            return ls.Count;
        }
         ...
         * @param g
         */
        public void printProbTable(float[,,] probTable, IList<string> words, ProbUnrestrictedGrammar g)
        {
            int N = words.Count;
            int M = g.vars.Count; // num non-terminals in grammar

            for (int i = 0; i < M; i++)
            {
                Console.WriteLine("Table For : " + g.vars[i] + "(" + i + ")");
                for (int j = 0; j < N; j++)
                {
                    Console.WriteLine(j + "| ");
                    for (int k = 0; k < N; k++)
                    {
                        Console.WriteLine(probTable[i, j, k] + " | ");
                    }
                    Console.WriteLine();
                }
                Console.WriteLine();
         ...
         * @return
         */
        public IList<string> getMostProbableDerivation(float[,,] probTable, ProbUnrestrictedGrammar g)
        {
            // TODO
            return null;
        }
