...
public class CYK {
	...
	public float[][][] parse( List<string> words, ProbCNFGrammar grammar ) {
		...
		final int M = grammar.vars.Count; 
	...
	public void printProbTable( float[][][] probTable, List<string> words, ProbUnrestrictedGrammar g ) {
		final int N = words.Count;
		final int M = g.vars.Count; // num non-terminals in grammar
		
		for( int i=0; i < M; ++i ) {
			Console.WriteLine("Table For : " + g.vars.get(i) + "(" + i + ")");
			for( int j=0; j < N; j++ ) {
				System.out.print(j + "| ");
				for( int k=0; k < N; k++ ) {
	 ...
	 * @return
	 */
	public List<string> getMostProbableDerivation( float[][][] probTable, ProbUnrestrictedGrammar g ) {
		// TODO
		return null;
	}
