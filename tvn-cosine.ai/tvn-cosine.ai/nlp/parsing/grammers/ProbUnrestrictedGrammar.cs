...
namespace tvn.cosine.ai.nlp.parsing.grammers
{
    ...
    { 
        ...
        public IList<string> vars;
        public IList<string> terminals;
        public int type;
        ...
        public virtual bool validateRuleProbabilities(IList<Rule> ruleList)
        {
            ...
            for (int i = 0; i < vars.Count; ++i)
            {
                for (int j = 0; j < ruleList.Count; j++)
                {
                    ...
                    if (ruleList[i].lhs[0].Equals(vars[i]))
                    {
                        probTotal += ruleList[i].PROB;
                    }
                    ...
                    if (j == ruleList.Count - 1 && probTotal != 1.0F)
                    {
                        return false;
                    }
                }
        ...
        public virtual void updateVarsAndTerminals(Rule r)
        {
            ...
            {
                if (isVariable(r.lhs[j]) && !vars.Contains(r.lhs[j]))
                {
                    vars.Add(r.lhs[j]);
                }
                else if (isTerminal(r.lhs[j]) && !terminals.Contains(r.lhs[j]))
                {
                    terminals.Add(r.lhs[j]);
                }
            }
            ...
            if (r.rhs != null)
            {
                ...
                {
                    if (isVariable(r.rhs[j]) && !vars.Contains(r.rhs[j]))
                    {
                        vars.Add(r.rhs[j]);
                    }
                    else if (isTerminal(r.rhs[j]) && !terminals.Contains(r.rhs[j]))
                    {
                        terminals.Add(r.rhs[j]);
                    }
                }
            }
            // maintain sorted lists
             (vars as List<string>).Sort();
            (terminals as List<string>).Sort();
        }
