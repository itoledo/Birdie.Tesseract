...
namespace tvn.cosine.ai.probability.bayes.approx
{
    ...
    public class LikelihoodWeighting : BayesSampleInference
    {
        ...
        public CategoricalDistribution likelihoodWeighting(RandomVariable[] X, AssignmentProposition[] e, BayesianNetwork bn, int N)
        {
            ...
            {
                // <b>x</b>,w <- WEIGHTED-SAMPLE(bn,e)
                Pair<IMap<RandomVariable, object>, double> x_w = weightedSample(bn, e);
                // W[x] <- W[x] + w where x is the value of X in <b>x</b>
         ...
         */
        public Pair<IMap<RandomVariable, object>, double> weightedSample(BayesianNetwork bn, AssignmentProposition[] e)
        {
            // w <- 1;
            double w = 1.0;
