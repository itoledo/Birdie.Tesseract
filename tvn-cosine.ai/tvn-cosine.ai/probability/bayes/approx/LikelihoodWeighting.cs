...
public class LikelihoodWeighting : BayesSampleInference {
	...
	public CategoricalDistribution likelihoodWeighting(RandomVariable[] X,
		...
		for (int j = 0; j < N; j++) {
			// <b>x</b>,w <- WEIGHTED-SAMPLE(bn,e)
			Pair<IDictionary<RandomVariable, Object>, double> x_w = weightedSample(bn,
					e);
			// W[x] <- W[x] + w where x is the value of X in <b>x</b>
	 ...
	 */
	public Pair<IDictionary<RandomVariable, Object>, double> weightedSample(
		...
		// return <b>x</b>, w
		return new Pair<IDictionary<RandomVariable, Object>, double>(x, w);
	}
