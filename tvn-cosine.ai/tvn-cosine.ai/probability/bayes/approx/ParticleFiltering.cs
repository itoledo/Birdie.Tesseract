 namespace aima.core.probability.bayes.approx;
...
public class ParticleFiltering {
	...
	public void initPersistent(int N, DynamicBayesianNetwork dbn) {
		...
		S = new AssignmentProposition[N][this.dbn.getX_0().Count];
		S_tp1 = new AssignmentProposition[N][this.dbn.getX_0().Count];
		Integer[] indexes = new Integer[N];
		for (int i = 0; i < N; ++i) {
			indexes[i] = i;
			IDictionary<RandomVariable, Object> sample = priorSampler
			...
			for (Map.Entry<RandomVariable, Object> sa : sample.entrySet()) {
				S[i][idx] = new AssignmentProposition(this.dbn.getX_0_to_X_1()
						.get(sa.Key), sa.getValue());
				S_tp1[i][idx] = new AssignmentProposition(this.dbn
						.getX_0_to_X_1().get(sa.Key), sa.getValue());
				idx++;
			}
		}
	...
	private void sampleFromTransitionModel(int i) {
		...
		IDictionary<RandomVariable, Object> x = new Dictionary<RandomVariable, Object>();
		for (int n = 0; n < S[i].Length; n++) {
			AssignmentProposition x1 = S[i][n];
			x.Add(this.dbn.getX_1_to_X_0().get(x1.getTermVariable()),
					x1.getValue());
		}
