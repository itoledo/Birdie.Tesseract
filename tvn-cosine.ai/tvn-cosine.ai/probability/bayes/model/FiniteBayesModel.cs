 namespace aima.core.probability.bayes.model;
...
public class FiniteBayesModel : FiniteProbabilityModel {
	...
	public bool isValid() {
		...
		return Math.Abs(1 - prior(representation
				.toArray(new Proposition[representation.Count]))) <= ProbabilityModel.DEFAULT_ROUNDING_THRESHOLD;
	}
	...
	public double prior(Proposition... phi) {
		...
		RandomVariable[] X = conjunct.getScope().toArray(
				new RandomVariable[conjunct.getScope().Count]);
	...
	public CategoricalDistribution jointDistribution(
		...
		if (vars.Count > 0) {
			...
			RandomVariable[] X = conjProp.getScope().toArray(
					new RandomVariable[conjProp.getScope().Count]);
