...
public class HMMExampleFactory {
	...
	public static HiddenMarkovModel getUmbrellaWorldModel() {
		Matrix transitionModel = new Matrix(new double[][] { { 0.7, 0.3 },
				{ 0.3, 0.7 } });
		IDictionary<Object, Matrix> sensorModel = new Dictionary<Object, Matrix>();
		sensorModel.Add(Boolean.TRUE, new Matrix(new double[][] { { 0.9, 0.0 },
				{ 0.0, 0.2 } }));
		sensorModel.Add(Boolean.FALSE, new Matrix(new double[][] {
				{ 0.1, 0.0 }, { 0.0, 0.8 } }));
