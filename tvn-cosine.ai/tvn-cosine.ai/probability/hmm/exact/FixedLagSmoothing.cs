...
namespace tvn.cosine.ai.probability.hmm.exact
{
     ...
     * 
     */
    public class FixedLagSmoothing<T>
    {
        ...
        // empty
        private List<Matrix> e_tmd_to_t = new List<Matrix>();
        // a hidden Markov model with S * S transition matrix <b>T</b>
        private HiddenMarkovModel<T> hmm = null;
        // d, the length of the lag for smoothing
        private int d = 1;
        //
         ...
         *            d, the length of the lag for smoothing
         */
        public FixedLagSmoothing(HiddenMarkovModel<T> hmm, int d)
        {
            this.hmm = hmm;
            this.d = d;
            initPersistent();
         ...
         */
        public CategoricalDistribution<T> fixedLagSmoothing(IList<AssignmentProposition<T>> et)
        {
            ...
            O_t = e_tmd_to_t[e_tmd_to_t.Count - 1];
            // if t > d then
            if (t > d)
            {
                // remove e<sub>t-d-1</sub> from the beginning of e<sub>t-d:t</sub>
            ...
            // if t > d then return NORMALIZE(<b>f</b> * <b>B1</b>) else return null
            CategoricalDistribution<T> rVal = null;
            if (t > d)
            {
        ...
        {
            return hmm.normalize(O_tp1.times(hmm.getTransitionModel()
                                                .transpose() 
                                                .times(f1_t)));
        }
