...
namespace tvn.cosine.ai.probability.hmm.exact
{
     ...
     */
    public class HMMForwardBackwardConstantSpace<T> : HMMForwardBackward<T>
    {
        public HMMForwardBackwardConstantSpace(HiddenMarkovModel<T> hmm)
                : base(hmm)
        { }

        //
        // START-ForwardBackwardInference 
        public override IList<CategoricalDistribution<T>> forwardBackward(IList<IList<AssignmentProposition<T>>> ev, CategoricalDistribution<T> prior)
        {
