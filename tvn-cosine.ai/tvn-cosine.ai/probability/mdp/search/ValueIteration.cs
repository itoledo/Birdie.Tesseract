 namespace aima.core.probability.mdp.search;
...
public class ValueIteration<S, A : Action> {
	...
	public ValueIteration(double gamma) {
		if (gamma > 1.0 || gamma <= 0.0) {
			throw new ArgumentException("Gamma must be > 0 and <= 1.0");
		}
		this.gamma = gamma;
	}
	 ...
	 */
	public IDictionary<S, double> valueIteration(MarkovDecisionProcess<S, A> mdp,
			double epsilon) {
		//
		// local variables: U, U', vectors of utilities for states in S,
		// initially zero
		IDictionary<S, double> U = Util.create(mdp.states(), new double(0));
		IDictionary<S, double> Udelta = Util.create(mdp.states(), new double(0));
