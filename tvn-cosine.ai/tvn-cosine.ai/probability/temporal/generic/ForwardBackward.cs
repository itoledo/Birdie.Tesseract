 namespace aima.core.probability.temporal.generic;
...
public class ForwardBackward : ForwardBackwardInference {
	...
	public List<CategoricalDistribution> forwardBackward(
		...
		// local variables: fv, a vector of forward messages for steps 0,...,t
		List<CategoricalDistribution> fv = new List<CategoricalDistribution>(
				ev.Count + 1);
		// b, a representation of the backward message, initially all 1s
		CategoricalDistribution b = initBackwardMessage();
		// sv, a vector of smoothed estimates for steps 1,...,t
		List<CategoricalDistribution> sv = new List<CategoricalDistribution>(
				ev.Count);
	...
	public CategoricalDistribution forward(CategoricalDistribution f1_t,
		...
		Proposition[] props = new Proposition[s1.getFor().Count];
		int i = 0;
		...
		final AssignmentProposition[] xt = new AssignmentProposition[tToTm1StateVarMap
				.Count];
		final IDictionary<RandomVariable, AssignmentProposition> xtVarAssignMap = new Dictionary<RandomVariable, AssignmentProposition>();
		i = 0;
		for (RandomVariable rv : tToTm1StateVarMap.Keys) {
		...
		CategoricalDistribution.Iterator if1_t = new CategoricalDistribution.Iterator() {
			public void iterate(IDictionary<RandomVariable, Object> possibleWorld,
				...
				for (Map.Entry<RandomVariable, Object> av : possibleWorld
						.entrySet()) {
					xtVarAssignMap.get(av.Key).setValue(av.getValue());
				}
				int i = 0;
	...
	public CategoricalDistribution backward(CategoricalDistribution b_kp2t,
		...
		final CategoricalDistribution b_kp1t = new ProbabilityTable(
				b_kp2t.getFor());
		// Set up required working variables
		Proposition[] props = new Proposition[b_kp1t.getFor().Count];
		int i = 0;
		...
		final AssignmentProposition[] ax_kp1 = new AssignmentProposition[tToTm1StateVarMap
				.Count];
		final IDictionary<RandomVariable, AssignmentProposition> x_kp1VarAssignMap = new Dictionary<RandomVariable, AssignmentProposition>();
		i = 0;
		for (RandomVariable rv : b_kp1t.getFor()) {
		...
		props = new Proposition[e_kp1.Count];
		...
		CategoricalDistribution.Iterator ib_kp2t = new CategoricalDistribution.Iterator() {
			public void iterate(IDictionary<RandomVariable, Object> possibleWorld,
				...
				for (Map.Entry<RandomVariable, Object> av : possibleWorld
						.entrySet()) {
					x_kp1VarAssignMap.get(av.Key).setValue(av.getValue());
				}
	...
	private CategoricalDistribution initBackwardMessage() {
		ProbabilityTable b = new ProbabilityTable(tToTm1StateVarMap.Keys);
