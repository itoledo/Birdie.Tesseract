...
public class ProbUtil {
	private static readonly Pattern LEGAL_RAND_VAR_NAME_PATTERN              = Pattern.compile("[A-Za-z0-9-_]+");
	private static readonly Pattern LEGAL_LEADING_CHAR_RAND_VAR_NAME_PATTERN = Pattern.compile("^[A-Z].*");
	...
	public static int expectedSizeOfProbabilityTable(RandomVariable... vars) {
		...
		if (null != vars) {
			for (RandomVariable rv : vars) {
				...
				expectedSizeOfDistribution *= d.Count;
			}
		}
	...
	public static object sample(double probabilityChoice, RandomVariable Xi,
		...
		if (fd.Count != distribution.Length) {
			throw new ArgumentException("Size of domain Xi " + fd.Count
	...
	public static object randomSample(Node Xi,
			IDictionary<RandomVariable, Object> event, Randomizer r) {
	...
	public static double[] mbDistribution(Node Xi,
			IDictionary<RandomVariable, Object> event) {
		FiniteDomain fd = (FiniteDomain) Xi.getRandomVariable().getDomain();
		double[] X = new double[fd.Count];
		...
		IDictionary<RandomVariable, Object> generatedEvent = new Dictionary<RandomVariable, Object>();
		for (Map.Entry<RandomVariable, Object> entry : event.entrySet()) {
	        generatedEvent.Add(entry.Key, entry.getValue());     
	    }
	 ...
	 */
	public static object[] getEventValuesForXiGivenParents(Node Xi,
			Object xDelta, IDictionary<RandomVariable, Object> event) {
	 ...
	 *         the values for X.
	 */
	public static int indexOf(RandomVariable[] X, IDictionary<RandomVariable, Object> x) {
		if (0 == X.Length) {
	...
	public static int[] indexesOfValue(RandomVariable[] X, int idx,
			IDictionary<RandomVariable, Object> x) {
		...
		int vdosize = fd.Count;
		int[] indexes = new int[csize / vdosize];

		int blocksize = csize;
		for (int i = 0; i < X.Length; ++i) {
			blocksize = blocksize / X[i].getDomain().Count;
			if (i == idx) {
				break;
			}
		}
