...
public class ProbabilityTable : CategoricalDistribution, Factor {
	...
	public interface Iterator {
		 ...
		 */
		void iterate(IDictionary<RandomVariable, Object> possibleAssignment,
				double probability);
	}

	public ProbabilityTable(ICollection<RandomVariable> vars) {
		this(vars.toArray(new RandomVariable[vars.Count]));
	}
	...
	public ProbabilityTable sumOut(RandomVariable... vars) {
		Set<RandomVariable> soutVars = new HashSet<RandomVariable>(
				this.randomVarInfo.Keys);
		for (RandomVariable rv : vars) {
			soutVars.Remove(rv);
		}
		...
		if (1 == summedOut.getValues().Length) {
			...
			ProbabilityTable.Iterator di = new ProbabilityTable.Iterator() {
				public void iterate(IDictionary<RandomVariable, Object> possibleWorld,
					...
					int i = 0;
					for (RandomVariable rv : summedOut.randomVarInfo.Keys) {
	...
	public void iterateOverTable(Iterator pti) {
		IDictionary<RandomVariable, Object> possibleWorld = new Dictionary<RandomVariable, Object>();
	...
	public void iterateOverTable(Iterator pti,
		...
		if (fixedValues.Length == randomVarInfo.Count) {
			...
			Set<RandomVariable> freeVariables = SetOps.difference(
					this.randomVarInfo.Keys, possibleWorld.Keys);
	...
	public ProbabilityTable divideBy(ProbabilityTable divisor) {
		if (!randomVarInfo.Keys.containsAll(divisor.randomVarInfo.Keys)) {
			throw new ArgumentException(
		...
		if (1 == divisor.getValues().Length) {
			...
			Set<RandomVariable> dividendDivisorDiff = SetOps
					.difference(this.randomVarInfo.Keys,
							divisor.randomVarInfo.Keys);
			...
			final IDictionary<RandomVariable, RVInfo> diff = tdiff;
			...
			ProbabilityTable.Iterator divisorIterator = new ProbabilityTable.Iterator() {
				public void iterate(IDictionary<RandomVariable, Object> possibleWorld,
						double probability) {
					for (RandomVariable rv : possibleWorld.Keys) {
					...
					if (null != diff) {
						...
						do {
							for (RandomVariable rv : diff.Keys) {
	...
	public ProbabilityTable pointwiseProduct(final ProbabilityTable multiplier) {
		Set<RandomVariable> prodVars = SetOps.union(randomVarInfo.Keys,
				multiplier.randomVarInfo.Keys);
		return pointwiseProductPOS(multiplier, prodVars
				.toArray(new RandomVariable[prodVars.Count]));
	}
	...
	public ProbabilityTable pointwiseProductPOS(
		...
		if (!product.randomVarInfo.Keys .Equals(
				SetOps.union(randomVarInfo.Keys, multiplier.randomVarInfo
						.Keys))) {
			throw new ArgumentException(
		...
		if (1 == product.getValues().Length) {
			...
			final object[] term1Values = new object[randomVarInfo.Count];
			...
			ProbabilityTable.Iterator di = new ProbabilityTable.Iterator() {
				...
				private int termIdx(object[] termValues, ProbabilityTable d,
						IDictionary<RandomVariable, Object> possibleWorld) {
					...
					int i = 0;
					for (RandomVariable rv : d.randomVarInfo.Keys) {
						termValues[i] = possibleWorld.get(rv);
						i++;
					}
