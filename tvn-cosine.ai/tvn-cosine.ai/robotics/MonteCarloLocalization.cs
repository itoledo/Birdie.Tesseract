 ...
 */
public final class MonteCarloLocalization<P : IMclPose<P,V,M>, V : IMclVector, M : IMclMove<M>, R : IMclRangeReading<R,V>> {
	
	private static readonly string SAMPLE_INDEXES_NAME = "SAMPLE_INDEXES";
	
	private readonly IMclIDictionary<P,V,M,R> map;
	 ...
	 */
	public MonteCarloLocalization(IMclIDictionary<P,V,M,R> map, Randomizer randomizer) {
		this.map = map;
	...
	protected ISet<P> applyMove(ISet<P> samples, M move) {
		Set<P> newSamples = new HashSet<P>();
	...
	protected double[] weightSamples(ISet<P> samples, R[] rangeReadings) {
		...
		double[] w = new double[samples.Count];
	...
	protected ISet<P> extendedWeightedSampleWithReplacement(ISet<P> samples, double[] w) {
		...
		if(i >= samples.Count) return generateCloud(samples.Count); /*If all particleCloud are below weightCutOff, generate a new set of samples, as we are lost.*/
